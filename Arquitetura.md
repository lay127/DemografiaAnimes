...
O sistema foi projetado para integrar diferentes fontes de dados relacionadas ao universo dos animes, realizando processamento, análise estatística e visualização interativa por meio de dashboards desenvolvidos em HTML.

Além das visualizações gráficas, os dashboards apresentam interpretações automáticas dos indicadores calculados, permitindo identificar padrões de subvalorização, distribuição desigual de visibilidade e comportamento do público.

Os dashboards seguem uma abordagem de **Data Storytelling**, combinando gráficos, métricas e interpretações textuais para facilitar a compreensão dos padrões identificados ao longo da análise.
...
O usuário interage diretamente com o sistema por meio de dashboards desenvolvidos em HTML.

O sistema depende de fontes externas para obtenção e enriquecimento dos dados:

* **Datasets do Kaggle**, utilizados como base principal para análise
* **API oficial do MyAnimeList**, utilizada via autenticação por Client ID para complementação e atualização das informações

Os dados coletados são processados e organizados para alimentar dois dashboards distintos:

* Um dashboard focado na análise de animes e métricas de popularidade
* Um dashboard focado no comportamento e perfil dos espectadores
...
### 🖥️ Dashboards em HTML

Responsáveis pela interface com o usuário, permitindo:

* Filtros por demografia
* Visualização de gráficos
* Comparações entre categorias
* Navegação interativa entre métricas
* Exibição de tabelas e rankings
* Apresentação de análises interpretativas automáticas

Além disso, serão desenvolvidos dois dashboards distintos, cada um baseado em um dataset diferente:

### 📊 Dashboard 1 — Dados do MyAnimeList
...
Este dashboard utiliza dados provenientes dos datasets do MyAnimeList e informações complementadas pela API oficial da plataforma.

### 👥 Dashboard 2 — Perfil dos Espectadores
...
* Consumo semanal
* Engajamento
* Gasto com merchandise
* Plataformas utilizadas
* Faixa etária dos espectadores
* Relação entre gênero do público e preferência demográfica

Esse dashboard busca identificar padrões estruturais relacionados à visibilidade e renovação de audiência do gênero Shoujo.
...
* Cálculo de métricas
* Geração de indicadores derivados
* Tratamento estatístico

O sistema realiza cálculos estatísticos e geração de métricas derivadas para apoiar análises comparativas entre demografias.
...
* Reutilização dos dados processados
...
* Realiza requisições na API oficial do MyAnimeList utilizando Client ID
...
* Calcula métricas como:
  ...
  * Rankings de visibilidade

### 📝 Interpretação dos Dados

Após o cálculo das métricas, o sistema gera interpretações textuais baseadas nos padrões encontrados.

Essas interpretações utilizam linguagem analítica e descritiva, destacando tendências observadas nos dados, como possíveis disparidades de visibilidade, diferenças de engajamento e padrões de consumo entre diferentes demografias.
...
Os dashboards foram desenvolvidos utilizando tecnologias web, permitindo maior flexibilidade na personalização da interface, interatividade e organização visual dos dados apresentados.
...
- API oficial do MyAnimeList

A utilização de datasets do Kaggle e da API oficial do MyAnimeList permite obter dados amplos e atualizados sobre animes, garantindo maior diversidade, completude e confiabilidade das informações utilizadas na análise.

### 💾 Armazenamento

- Arquivos CSV armazenados localmente ou no GitHub

A utilização de arquivos CSV permite versionamento dos dados e fácil integração com os dashboards desenvolvidos em HTML.
...
A escolha do desenvolvimento de dashboards em HTML como camada de visualização permite maior flexibilidade na construção da interface e maior controle sobre os elementos gráficos e interativos da aplicação.

Além das visualizações tradicionais, a arquitetura também suporta a geração de interpretações analíticas automáticas, permitindo transformar métricas estatísticas em explicações textuais que auxiliam na compreensão dos resultados.

Isso possibilita que os dados sejam previamente tratados em Python e posteriormente consumidos pelos dashboards, reduzindo o custo computacional durante a interação e melhorando o desempenho geral da aplicação.

O armazenamento dos dados processados em arquivos CSV foi adotado por sua simplicidade e eficiência. Essa abordagem dispensa o uso de um banco de dados dedicado, ao mesmo tempo em que garante facilidade de manipulação, versionamento e compartilhamento dos dados.
...
* os resultados precisam ser comunicados de forma narrativa e interpretativa.
...
A divisão em containers (dashboards, processamento e armazenamento) segue o princípio de separação de responsabilidades, permitindo maior organização, manutenção e escalabilidade do sistema.
...
* Integração com bibliotecas gráficas JavaScript
* Novos dashboards especializados
...
A solução proposta consiste no desenvolvimento de dois dashboards interativos em HTML, cada um baseado em um dataset diferente obtido no Kaggle.

Os dashboards permitirão a visualização e exploração de métricas relacionadas aos animes, possibilitando comparações entre demografias, análises de popularidade, avaliações e padrões de distribuição dos dados.

Além disso, a utilização de HTML possibilita maior flexibilidade na personalização da interface, interatividade e organização visual das informações apresentadas.
...
A API oficial do MyAnimeList será utilizada para complementar e enriquecer os dados obtidos nos datasets do Kaggle, utilizando autenticação via Client ID fornecida pela plataforma.

Essas fontes fornecem dados sobre animes, popularidade, avaliações, número de membros, temporadas, gêneros e outras informações relevantes para as análises propostas.

Os dados passarão por etapas de limpeza, integração e enriquecimento para viabilizar a análise proposta.
